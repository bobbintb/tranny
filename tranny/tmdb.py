...
def configure(api_key, language='en'):
    ...
    config['urls'] = {}
    config['urls'][
        'movie.search'] = "https://api.themoviedb.org/3/search/movie?query=%%s&api_key=%(apikey)s&page=%%s" % (config)
            ...
            if int(pages) > 1:
                for i in range(2, int(pages) + 1):
                    self.people["results"].extend(
                        self.getJSON(config['urls']['people.search'] % (people_name, str(i)), language=language)[
                            "results"])
