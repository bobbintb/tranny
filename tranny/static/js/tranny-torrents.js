...
(function() {
  var STATUS_CLIENT_NOT_AVAILABLE, STATUS_FAIL, STATUS_INCOMPLETE_REQUEST, STATUS_INTERNAL_ERROR, STATUS_INVALID_INFO_HASH, STATUS_OK, TorrentTable, action_reannounce, action_recheck, action_remove, action_remove_data, action_start, action_stop, action_torrent_details, action_torrent_peers, action_torrent_speed, add_class, bytes_to_iec_size, bytes_to_size, detail_elements, detail_traffic_chart, detail_update_speed, detail_update_timer, endpoint, error_handler, fmt_duration, fmt_timestamp, handle_event_alert, handle_event_speed_overall_response, handle_event_torrent_details_response, handle_event_torrent_files_response, handle_event_torrent_list_response, handle_event_torrent_peers_response, handle_event_torrent_reannounce_response, handle_event_torrent_recheck_response, handle_event_torrent_remove_response, handle_event_torrent_speed_response, handle_event_torrent_stop_response, has_class, has_connected, in_url, init_context_menu, init_provider_totals_chart, init_provider_type_totals_chart, init_section_totals_chart, init_traffic_chart, make_pie_chart, overall_speed_update, overall_speed_update_timer, peer_update_timer, rand_int, remove_class, render_peers, root, row_load_handler, row_remove, selected_class, selected_detail_id, selected_rows, show_alert, socket, special_alerts, speed_data, speed_dn, speed_up, speed_update_timer, template, toggle_class, torrent_table, torrent_wrapper, ts, update_speed, user_messages, window_resize_handler, _alert_num, _chart_interval_timer, _iec_sizes, _sizes,
  ...
  */
  ...
  TorrentTable = (function() {
    ...
    function TorrentTable(dom_id) {
      ...
      this.cols = ['name', 'size', 'progress', 'ratio', 'up_rate', 'dn_rate', 'seeders', 'peers', 'priority', 'is_active'];
      this.info_hashes = {};
    }
  ...
  */
  ...
  handle_event_torrent_peers_response = function(message) {
    ...
    peer_chart_data = [];
    client_chart_data = [];
    _ref = _.countBy(message['data'], 'country');
    for (country in _ref) {
      count = _ref[country];
    ...
    _ref1 = _.countBy(message['data'], sort_client);
    for (client in _ref1) {
      count = _ref1[client];
    ...
    return render_peers(message['data']);
  };
  ...
  handle_event_torrent_details_response = function(message) {
    ...
    detail_elements.detail_seeding_time.text(data['seeding_time'] ? fmt_duration(data['seeding_time']) : "n/a");
    ...
    return detail_elements.detail_tracker.text(data['tracker_status']);
  };
